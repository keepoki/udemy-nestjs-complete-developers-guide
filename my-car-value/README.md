...
쿼리 문자열의 자동 형변환을 위해 `Transform` 데코레이터를 사용하였다.

### 149. 견적을 생성하는 방법

견적을 생성해야 하는데, 현재는 견적을 정확하게 계산하기 보다 중요한 것은 TypeORM의 내부 원리와 이를 통해 유용한 쿼리를 실행하는 방법을 이해하는 것이다. 실제 쿼리의 유효성보다 쿼리를 실행하는 방법에 중점을 둔다.

1. 견적 기준을 선정한다.
2. 데이터베이스에 있는 모든 보고서를 검색한다. 
    1. 제조사와 모델이 모두 일치하는지 확인한다.
    2. 위도와 경도가 +-5도 범위에 있는지 확인한다. (300마일 거리)
    3. 생산년도가 +-3년 범위에 있는지 확인한다.
    4. 가장 가까운 마일리지 값을 찾는다.
3. 그 중 가장 가까운 상위 3개의 보고서를 선택해 값의 평균을 구한다.
4. 평균 값을 사용자에게 제공한다.

## Section 17. TypeORM을 이용한 쿼리 빌더

### 150. 쿼리 빌더 만들기

### 151. 견적 생성을 위한 쿼리 작성하기
...
### 155. synchronize 플래그 이해하기

### 156. synchronize의 위험성

### 157. 마이그레이션 이론

### 158. 설정 관리의 어려움

### 159. TypeORM과 NestJS의 설정 연동

### 160. 환경별 데이터베이스 설정

### 161. TypeORM CLI 설치하기

### 162. 마이그레이션 생성하고 실행하기

### 163. 프로덕션 환경에 필요한 마이그레이션 업데이트

### 164. E2E 테스트 시 마이그레이션 실행하기

### 165. 운영 환경 DB 설정

### 166. Heroku CLI 셋업 안내
