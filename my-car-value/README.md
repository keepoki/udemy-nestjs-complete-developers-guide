...
- **Section 17. TypeORM을 이용한 쿼리 빌더**
  - [150. 쿼리 빌더 만들기](#150-쿼리-빌더-만들기)
  - [151. 견적 생성을 위한 쿼리 작성하기](#151-견적-생성을-위한-쿼리-작성하기)
  - [152. 견적 논리 테스트하기](#152-견적-논리-테스트하기)
- **Section 18. 운영 환경 배포**
  - [153. 운영 환경 배포 과정](#153-운영-환경-배포-과정)
  - [154. 쿠키 키 설정하기](#154-쿠키-키-설정하기)
  - [155. synchronize 플래그 이해하기](#155-synchronize-플래그-이해하기)
  - [156. synchronize의 위험성](#156-synchronize의-위험성)
  - [157. 마이그레이션 이론](#157-마이그레이션-이론)
  - [158. 설정 관리의 어려움](#158-설정-관리의-어려움)
  - [159. TypeORM과 NestJS의 설정 연동](#159-typeorm과-nestjs의-설정-연동)
  - [160. 환경별 데이터베이스 설정](#160-환경별-데이터베이스-설정)
  - [161. TypeORM CLI 설치하기](#161-typeorm-cli-설치하기)
  - [162. 마이그레이션 생성하고 실행하기](#162-마이그레이션-생성하고-실행하기)
  - [163. 프로덕션 환경에 필요한 마이그레이션 업데이트](#163-프로덕션-환경에-필요한-마이그레이션-업데이트)
  - [164. E2E 테스트 시 마이그레이션 실행하기](#164-e2e-테스트-시-마이그레이션-실행하기)
  - [165. 운영 환경 DB 설정](#165-운영-환경-db-설정)
  - [166. Heroku CLI 셋업 안내](#166-heroku-cli-셋업-안내)
  - [167. Heroku용 프로젝트 설정](#167-heroku용-프로젝트-설정)
  - [168. 앱 배포하기 (마지막 강의)](#168-앱-배포하기-마지막-강의)
...
견적을 생성하는 방법에 대한 모든 필터링 단계를 실행한 후 순서대로 정렬하는 논리를 실행하기 위해 쿼리 빌더를 사용한다.

`createQueryBuilder`를 사용하여 쿼리 빌더를 생성한다.

임시적으로 쿼리 빌더를 통해 데이터베이스에 있는 모든 보고서를 검색하는 것으로 설정하였다.

### 151. 견적 생성을 위한 쿼리 작성하기

쿼리 빌더는 복잡한 쿼리를 생성하는 데 매우 유용하다.

`:`구문을 사용해서 검색 매개변수를 대체하는 이유는 보안 때문이다. SQL 주입 공격을 방지한다.
